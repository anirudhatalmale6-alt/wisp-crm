...
class MikroTik {
  ...
  async addToBlockList(ipAddress, comment) {
    ...
    const existing = await this.exec(`/ip firewall address-list print where list=morosos address="${ipAddress}"`);
    ...
    await this.exec(`/ip firewall address-list add list=morosos address=${ipAddress} comment="${safeComment}"`);
    return { success: true, message: `IP ${ipAddress} agregada a lista de morosos` };
  }

  async removeFromBlockList(ipAddress) {
    try {
      await this.exec(`/ip firewall address-list remove [find list=morosos address="${ipAddress}"]`);
